...
func spawn_enemies(row_info: pregenerated_wave) -> void:
	for n in row_info.wave.size():
		...
		enemy_object.global_position = Vector2(grid_to_global(Vector2(x,0)).x,y - ((_spawn_area.size.y / globals.lanes)*3))
...
func _move_enemies() -> void:
	if spawn_parent.get_child_count() > 0:
		for child in spawn_parent.get_children():
			...
			child.danger_check(globals.ranks-1)
	game_over_check()
...
func grid_to_global(coordinates: Vector2) -> Vector2:
	var percent_x = coordinates.x / globals.lanes
	# var percent_y = coordinates.y / globals.ranks
	var startPos: Vector2
	var endPos: Vector2
	var output = Vector2.ZERO
	startPos.x = _spawn_area.global_position.x + ((_spawn_area.size.x / globals.lanes)*coordinates.x) + ((_spawn_area.size.x / globals.lanes)/2)
	endPos.x = _end_area.global_position.x + ((_end_area.size.x / globals.lanes) * coordinates.x)+ ((_spawn_area.size.x / globals.lanes)/2)
	startPos.y = _spawn_area.global_position.y
	endPos.y = _end_area.global_position.y + _end_area.size.y

	var laneHeight = (endPos.y - startPos.y) / globals.ranks
...
func game_over_check():
	for child in spawn_parent.get_children():
		if child.game_over_check(globals.ranks-1):
